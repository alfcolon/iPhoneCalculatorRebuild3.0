...
enum CalculatorFunction: String {
	...
	enum SwitchableScientificFunctions {
		case first
		case second
		...
		mutating func toggle() {
			switch self {
			case .first:
				self = SwitchableScientificFunctions.second
			case .second:
				self = SwitchableScientificFunctions.first
			}
		}
	}
	...
	static func defaultLayout(for calculator: CalculatorMode) -> [CalculatorFunction] {
	...
	func itemNumber(for calculator: CalculatorMode) -> Int? {
		switch calculator {
		case .scientific:
			switch self {
			case .allClear:
