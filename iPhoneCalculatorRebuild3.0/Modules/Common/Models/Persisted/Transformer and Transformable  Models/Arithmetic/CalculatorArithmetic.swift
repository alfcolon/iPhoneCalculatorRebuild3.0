	...
	enum ArithmeticUpdate {
		...
		enum BasicOperatorUpdate: DisplayTermUpdateProtocol {
			...
			static func arithmeticUpdate(for precedenceOperation: PrecedenceOperation, basicOperator: CalculatorFunction) -> BasicOperatorUpdate {
				...
				let input2IsEmptyParentheticalExpressionContainer: Bool = {
					guard precedenceOperation.reverseParentheticalExpression?.reverseParentheticalExpressionContainer?.reverseFunctionTwoInputs != nil else { return false }
					guard precedenceOperation.reverseParentheticalExpression!.reverseParentheticalExpressionContainer.isEmpty else { return false }
					return true
				}()
				let input2IsEmpty: Bool = termToUpdate?.termType == .functionTwoInputs && (termToUpdate as? FunctionTwoInputs)?.input2 == nil
				let parentheticalExpressionIsEmpty: Bool = precedenceOperation.reverseParentheticalExpression!.isEmpty
				...
				case precedenceOperation.reverseParentheticalExpression?.currentPrecedence == .high:
					switch true {
				...
				case precedenceOperation.reverseParentheticalExpression?.currentPrecedence == .low:
					switch true {
		...
		enum ClosingParenthesisUpdate: DisplayTermUpdateProtocol {
			...
			static func arithmeticUpdate(for precedenceOperation: PrecedenceOperation) -> ClosingParenthesisUpdate {
				let isInput2 = precedenceOperation.reverseParentheticalExpression?.reverseParentheticalExpressionContainer.reverseFunctionTwoInputs != nil
				let input2IsEmpty = (precedenceOperation.reverseParentheticalExpression?.reverseParentheticalExpressionContainer.isEmpty)! && precedenceOperation.reverseParentheticalExpression?.reverseParentheticalExpressionContainer.parentheticalExpressions.count == 1
				...
				let noAddedParentheticalExpressions: Bool = {
					guard precedenceOperation.reverseParentheticalExpression?.reverseParentheticalExpressionContainer.reverseFunctionTwoInputs == nil else { return false }
					return precedenceOperation.reverseParentheticalExpression?.reverseParentheticalExpressionContainer.parentheticalExpressions.count == 1
				}()
