...
public class ParentheticalExpression: NSObject, NSSecureCoding, TermProtocol {
	...
	var low: PrecedenceOperation! { didSet { self.low.reverseParentheticalExpression = self }}
	var high: PrecedenceOperation! { didSet { self.high.reverseParentheticalExpression = self }}
	...
	init(container: ParentheticalExpressionContainer, startingTerm: TermProtocol?) {
		...
		super.init()
		self.low = PrecedenceOperation(container: self, leftTerm: startingTerm)
		self.high = PrecedenceOperation(container: self)
	...
	func removeCurrentPrecedenceOperation() -> PrecedenceOperation {
		...
		case .low:
			...
			self.low = PrecedenceOperation(container: self)
	...
	func refactorPrecedenceToLeftTerm(precedence: PrecedenceOperationType, arithmenticOperator: CalculatorFunction) {
		...
		case .high:
			...
			let doubleNumber = DoubleNumber(term: self.high)
			self.high = PrecedenceOperation(container: self, leftTerm: doubleNumber, rightTerm: nil, basicOperator: arithmenticOperator)
		case .low:
			...
			self.low = PrecedenceOperation(container: self, leftTerm: newLeftTermOperation, basicOperator: arithmenticOperator)
		}
	}
