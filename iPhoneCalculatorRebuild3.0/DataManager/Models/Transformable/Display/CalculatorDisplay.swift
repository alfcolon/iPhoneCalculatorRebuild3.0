...
public class CalculatorDisplay: NSObject, NSSecureCoding {
    ...
    public required init?(coder: NSCoder) {
        self.copiedNumber = {
            let term: TermProtocol? = {
                guard let termType: Term = {
                    guard let number = coder.decodeObject(of: NSNumber.self, forKey: Key.copiedNumberTermType.rawValue) else { return nil }
        ...
        self.displayNumber = {
            let term: TermProtocol = {
                guard let termType: Term = {
                    guard let number = coder.decodeObject(of: NSNumber.self, forKey: Key.displayNumberTermType.rawValue) else { fatalError() }
                    guard let termType = Term.init(rawValue: number.intValue) else { fatalError() }
                    return termType
                }() else { fatalError() }
